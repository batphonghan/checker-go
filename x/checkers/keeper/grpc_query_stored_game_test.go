...
func TestStoredGameQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:     "First",
			request:  &types.QueryGetStoredGameRequest{Index: msgs[0].Index},
			response: &types.QueryGetStoredGameResponse{StoredGame: &msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetStoredGameRequest{Index: msgs[1].Index},
			response: &types.QueryGetStoredGameResponse{StoredGame: &msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetStoredGameRequest{Index: "missing"},
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			...
			for j := i; j < len(msgs) && j < i+step; j++ {
				assert.Equal(t, &msgs[j], resp.StoredGame[j-i])
			}
		}
	})
